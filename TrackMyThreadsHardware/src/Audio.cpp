...
#define AUDIO_INFO(...) { snprintf(m_ibuff, m_ibuffSize, __VA_ARGS__); if(audio_info) audio_info(m_ibuff); }
    ...
    esp_err_t err = i2s_new_channel(&m_i2s_chan_cfg, &m_i2s_tx_handle, NULL);
	if (err != ESP_OK) {
	    Serial.printf("Failed to create I2S channel: %d\n", err);
	    m_i2s_tx_handle = NULL; // Optional: prevent future crashes
	}  
...
bool Audio::latinToUTF8(char* buff, size_t bufflen, bool UTF8check) {
    ...
    if(UTF8check){
        while(pos < len) {  // check first, if we have a clear UTF-8 string
            c = buff[pos];
            if(c >= 0xC2 && c <= 0xDF) { // may be 2 bytes UTF8, e.g. 0xC2B5 is 'Âµ' (MICRO SIGN)
                if(pos + 1 == len){
                    isUTF8 = false;
                    break;
                }
            ...
            if(c >= 0xE0 && c <= 0xEF){ // may  be 3 bytes UTF8, e.g. 0xE0A484 is 'à¤„' (DEVANAGARI LETTER SHORT A)
                if(pos + 2 >= len){ //
                    isUTF8 = false;
                    break;
                }
...
int Audio::read_ID3_Header(uint8_t* data, size_t len) {
    ...
    if(m_controlCounter == 6) { // Read the value
        ...
        // $00 â€“ ISO-8859-1 (LATIN-1, Identical to ASCII for values smaller than 0x80).
        // $01 â€“ UCS-2 encoded Unicode with BOM (Byte Order Mark), in ID3v2.2 and ID3v2.3.
        // $02 â€“ UTF-16BE encoded Unicode without BOM (Byte Order Mark) , in ID3v2.4.
...
result:     m_linesWithSeqNr[0] = #EXTINF:3.008,#MY-USER-CHUNK-DATA-1:ON-TEXT-DATA="20250316100954"media-ur748eh1d_b192000_227213779.aac
    ...
    // Look for differences from right:                                                    âˆ¨
...
void Audio::processWebStreamTS() {
    ...
    // first call, set some values to default â€”â€”â€”â€”â€”â€”â€”â€”â€”â€”â€”â€”â€”â€”â€”â€”â€”â€”â€”â€”â€”â€”â€”â€”â€”â€”â€”â€”â€”â€”â€”â€”â€”â€”â€”
    if(m_f_firstCall) {   // runs only ont time per connection, prepare for start
        m_f_firstCall = false;
        m_f_m3u8data = true;
        f_firstPacket = true;
    ...
    if(idx1 >= 0) {
        if(indexOf(ml, "xml version=", 7) > 0) {
            /* e.g. xmlStreamTitle
                  ...
                  </END_TIME_UTC><SHOW_NAME>Cidade</SHOW_NAME><SHOW_HOURS>22h Ã s 07h</SHOW_HOURS><SHOW_PANEL>0</SHOW_PANEL>
...
bool Audio::ts_parsePacket(uint8_t* packet, uint8_t* packetStart, uint8_t* packetLength) {
    ...
    // AFC  Adaptation field control, 01 â€“ no adaptation field, payload only, 10 â€“ adaptation field only, no payload,
    //                                11 â€“ adaptation field followed by payload, 00 â€“ RESERVED for future use
